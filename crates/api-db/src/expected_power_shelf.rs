...
pub async fn find(
    txn: &mut PgConnection,
    req: &rpc::forge::ExpectedPowerShelfRequest,
) -> DatabaseResult<Option<ExpectedPowerShelf>> {
    if let Some(ref uuid_val) = req.expected_power_shelf_id {
        let id = Uuid::parse_str(&uuid_val.value).map_err(|e| {
            DatabaseError::InvalidArgument(format!("invalid expected_power_shelf_id: {e}"))
        })?;
        find_by_id(txn, id).await
    } else {
        let mac = MacAddress::try_from(req.bmc_mac_address.as_str())
            .map_err(|e| DatabaseError::InvalidArgument(format!("invalid bmc_mac_address: {e}")))?;
