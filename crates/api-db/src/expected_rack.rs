...
pub async fn delete(rack_id: RackId, txn: &mut PgConnection) -> DatabaseResult<()> {
...
pub async fn update<'a>(
    expected_rack: &'a mut ExpectedRack,
    txn: &mut PgConnection,
    rack_type: String,
    metadata: Metadata,
) -> DatabaseResult<&'a mut ExpectedRack> {
    let query = "UPDATE expected_racks SET rack_type=$1, metadata_name=$2, metadata_description=$3, metadata_labels=$4 WHERE rack_id=$5 RETURNING rack_id";
    ...
    expected_rack.rack_type = rack_type;
    expected_rack.metadata = metadata;
    Ok(expected_rack)
}
