...
async fn test_db_create_and_find(pool: sqlx::PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let created =
        db::expected_rack::create(&mut txn, rack_id, "NVL72".to_string(), metadata).await?;
...
async fn test_db_update(pool: sqlx::PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut expected_rack = db::expected_rack::find_by_rack_id(&mut txn, ids[0])
        .await?
        .expect("Expected rack not found");

    assert_eq!(expected_rack.rack_type, "NVL72");

    let new_metadata = model::metadata::Metadata {
        name: "updated-rack".to_string(),
        description: "Updated description".to_string(),
        labels: Default::default(),
    };

    db::expected_rack::update(
        &mut expected_rack,
        &mut txn,
        "NVL36".to_string(),
        new_metadata,
    ...
    assert_eq!(expected_rack.rack_type, "NVL36");
    assert_eq!(expected_rack.metadata.name, "updated-rack");
...
async fn test_db_delete(pool: sqlx::PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    db::expected_rack::delete(ids[0], &mut txn).await?;
    txn.commit().await?;
...
async fn test_db_delete_nonexistent(pool: sqlx::PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = db::expected_rack::delete(rack_id, &mut txn).await;
...
async fn test_add_expected_rack_missing_rack_id(pool: sqlx::PgPool) {
    ...
    assert!(
        err.message().contains("rack_id is required"),
...
async fn test_add_expected_rack_creates_rack_entry(pool: sqlx::PgPool) {
    ...
    // Verify the rack was also created in the racks table with the embedded capabilities.
    let rack = db::rack::get(&pool, rack_id).await.unwrap();
    let capabilities = rack
        .config
        .rack_capabilities
        .expect("should have rack_capabilities");
    assert_eq!(capabilities.compute.count, 18);
}
