...
async fn get_expected_switch_1(txn: &mut PgConnection) -> Option<ExpectedSwitch> {
    let fixture_mac_address = "0a:0b:0c:0d:0e:0f".parse().unwrap();

    db::expected_switch::find_by_bmc_mac_address(txn, fixture_mac_address)
        .await
        .unwrap()
}
...
async fn test_duplicate_fail_create(pool: sqlx::PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let switch = get_expected_switch_1(&mut txn)
        .await
        .expect("Expected switch not found");
...
async fn test_update_bmc_credentials(pool: sqlx::PgPool) -> Result<(), Box<dyn std::error::Error>> {
    let mut txn = pool
        ...
        .expect("unable to create transaction on database pool");
    create_expected_switches(&mut txn).await;
    let mut switch = get_expected_switch_1(&mut txn)
        .await
        .expect("Expected switch not found");
...
async fn test_delete(pool: sqlx::PgPool) -> () {
    ...
    assert!(get_expected_switch_1(&mut txn).await.is_none())
}
